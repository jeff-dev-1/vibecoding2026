...
- **GitHub Actions**（`.github/workflows/ci.yml`）— 仓库 CI。
- **CD 流水线**（容器化构建 + 部署）— 部署后跑安全 Stage 链：
  `Build → Docker Build/Push → Deploy → Health Check → Smoke(DeepSeek 端到端) → Red Team → Pentest(DAST) → Supply Chain Gate → Notify`
